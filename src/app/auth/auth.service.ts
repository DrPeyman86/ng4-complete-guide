...
export class AuthService {
    ...
    logout() {
        //after adding Auth Store replace code with following
    ...
    autoLogin() {
        ...
        if(loadedUser.token) {
            //after adding auth store replace code below with
    ...
    private handleAuthentication(email: string,userId:string, token: string, expiresIn: number) {
        ...
        //once you add Auth Store, you can replace line below with following line
        // this.user.next(user);
        this.store.dispatch(
          new AuthActions.Login({
            email: email,
